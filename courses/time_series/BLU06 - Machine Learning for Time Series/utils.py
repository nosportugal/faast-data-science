...
def load_airline_data():
    airlines = pd.read_csv("data/AirPassengers.csv", index_col="Month")
...
def prepare_for_prediction(
    ...
    # build the target
    data_with_target = build_target(series_, number_of_periods_ahead, target)

    # build the features
    data_with_target_and_features = build_some_features(
        data_with_target,
        target,
    ...
    X_train, y_train, X_last_period = separate_train_and_test_set(
        last_period, training_data
    )
...
def predict_period_n(
    ...
    X_train, y_train, X_last_period = prepare_for_prediction(
        series_,
        number_of_periods_ahead,
        num_periods_lagged,
        num_periods_diffed,
...
def build_some_features(
    ...
    for i in range(1, num_periods_lagged + 1):
        ...
        df_[f"lagged_{i}"] = df_[target].shift(i)
    ...
    if holidays:
        holidays = df_[
            ((df_.index.month == 12) & (df_.index.day == 25))
            | ((df_.index.month == 1) & (df_.index.day == 1))
        ][target]
